...
type ChecklistItem = { id: string | number; text: string; done: boolean };
...
export function Checklist({
  ...
  const [localItems, setLocalItems] = useState<ChecklistItem[]>(() =>
    TASKS.map((text, index) => ({ id: index, text, done: false })));
  ...
  const spare = !controlled && items.length < MAX;
  ...
  useEffect(() => {
    ...
    const id = window.setTimeout(
      () => setLocalItems(TASKS.map((text, index) => ({ id: index, text, done: false }))),
      HOLD,
    );
  ...
  const add = () => {
    ...
    setLocalItems((v) => [...v, { id: `${Date.now()}-${v.length}`, text, done: false }]);
  };
    ...
    >
      ...
      {items.map((task, i) => (
        <Row
          ...
          onToggle={() => {
            if (controlled) onToggle?.(task.id);
            else setLocalItems((v) =>
...
function Row({
  ...
  return (
    <motion.button
      ...
      onClick={onToggle}
      style={{ height: ROW, zIndex: layer, "--slip": `${inset}px` } as CSSProperties}
